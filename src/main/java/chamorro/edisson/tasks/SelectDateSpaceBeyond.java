...
import static chamorro.edisson.userinterfaces.PageSpaceBeyond.DATE_DAY;
...
public class SelectDateSpaceBeyond implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(SelectDateCalendar.ultilMonth(dateOnScreen,clickNext,
                        SplitDate.getMonth(travellData.get(key)), SplitDate.getYear(travellData.get(key))),
                Click.on(DATE_DAY.of(SplitDate.getDay(travellData.get(key)))),Click.on(ok));

    }
    public static SelectDateSpaceBeyond with(DataTable travellData,String key, Target dateOnScreen,Target clickNext , Target ok){
